...
- [ ] After a file is selected, an "Upload" or "Vectorize" button triggers the POST `/upload-notes` request
- [ ] A progress bar appears and updates as SSE `progress` values arrive
- [ ] Progress bar reaches 100% and then hides (or shows a success state) when the `done: true` event arrives
